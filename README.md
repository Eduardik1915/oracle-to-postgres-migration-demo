 ...
 **Paskaidrojums:**  PostgreSQL ar IDENTITY var izmantot tikai `SMALLINT`, `INTEGER` vai `BIGINT`. `SMALLINT` būtu par mazu priekš ID, bet `BIGINT` paredzēts ļoti lieliem datu apjomiem.
...
**Paskaidrojums:** PostgreSQL atbalsta tieši tādu pašu sintaksi kā Oracle, bet tika nolemts `UNIQUE CONSTRAINT` izveidot atsevišķi,
...
**Paskaidrojums:** Oracle tips DATE un PostgreSQL tips TIMESTAMP būtiski neatšķiras — abi glabā datumu un laiku.
                   Galvenā atšķirība ir precizitātē: Oracle DATE satur laiku līdz sekundēm, bet PostgreSQL TIMESTAMP nodrošina laiku ar mikrosekunžu precizitāti (līdz 6 cipariem pēc komata).
...
**Paskaidrojums:** Oracle tips NUMBER un PostgreSQL tips NUMERIC ir līdzvērtīgi — abi ļauj precīzi uzglabāt gan veselus, gan decimāldaļskaitļus jebkurā izmērā.
                   Tomēr praksē PostgreSQL bieži ieteicams lietot arī INTEGER, BIGINT vai DECIMAL, ja skaidri zināms datu veids un diapazons.
...
**Paskaidrojums:** Oracle IN/OUT liekas pēc mainīgā nosaukuma, bet PostgreSQL pirms.
                   Pēc noklusējuma abi dialekti pieņem, ka parametri ir IN, tāpēc IN var arī neuzrādīt.
...
**Paskaidrojums:** Abi apzīmē funkcijas atgriežamās vērtības tipu.
                   Atšķirība tikai sintaksē — Oracle lieto vienskaitli `RETURN`, bet PostgreSQL daudzskaitli `RETURNS`.
                   No semantikas viedokļa tie darbojas identiski.
...
**Paskaidrojums:** Oracle gadījumā tukša virkne ('') tiek interpretēta kā NULL, tāpēc pietiek pārbaudīt ar `IS NULL`.
                   Savukārt PostgreSQL tukša virkne nav `NULL`, tā ir atsevišķa vērtība (''), tāpēc pārbaude jāveic tieši ar = '' vai `NULLIF()`.

---

### ↩️ Pāreja uz jaunu rindu
 - **Oracle**: `CHR(10)`
 - **PostgreSQL**: `E'\n'`
